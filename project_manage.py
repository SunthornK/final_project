...
class Student:
    ...
    def accept_request(self, request_id):
        request = self.my_db.search('member_pending_request').filter(
            lambda r: r['to_be_member'] == request_id and r['ProjectID'] == project_id
        )
        if request:
            ...
            project = self.my_db.search('project').filter(
                lambda p: p['ProjectID'] == project_id
            )
            if project:
    ...
    def deny_request(self, request_id):
        request = self.my_db.search('member_pending_request').filter(
            lambda r: r['to_be_member'] == request_id and r['to_be_member'] == self.user_id
        )
        if request:
    ...
    def create_project_and_become_lead(self, project_name):
        ...
        new_project = {
            "project_id": len(self.my_db.search('project').table) + 1,
            "project_name": project_name,
            "lead_id": self.user_id,
            "members": []
        }
        self.my_db.search('project').insert(new_project)

        # Update the Login table to mark the user as a project lead
        self.my_db.search('login').update('ID', self.user_id, {'role': 'lead'})

        print(f"Project '{project_name}' created, and {self.user_id} is now the lead.")

    def deny_all_member_requests(self):
        if user_role == "student":
            pending_requests = self.my_db.search('member_pending_request').table
    ...
    def send_member_requests(self, project_id, member_ids):
        ...
        project = self.my_db.search('project').filter(lambda p: p['project_id'] == project_id)
        if project and project[0]['lead_id'] == self.user_id:
            for member_id in member_ids:
                # Check if the member is a student (not a member or lead)
                member = self.my_db.search('login').filter(lambda m: m['ID'] == member_id and m['role'] == 'student')
                if member:
                    # Add request to Member_pending_request table
                    new_request = {
                        "request_id": len(self.my_db.search('member_pending_request').table) + 1,
                        "ProjectID": project_id,
                        "to_be_member": member_id,
                        "Response": "Pending",
                        "Response_date": "Pending"
                    }
                    self.my_db.search('member_pending_request').insert(new_request)
